...
- 支持上传、拖动、滑杆精调、鼠标滚轮/触控板和双指捏合缩放、底色预览与 JPEG 导出
...
- 自动抠图开关会区分未上传、处理中、可用、处理失败和规格禁止状态；临时加载失败可在下次上传时重试
- 支持蒙版擦除、原图恢复、画笔大小、最多 10 步撤销和一键重置
- 每条规格显示状态、官方来源及最后核实日期
- 页面底部单独标明暂不支持的目的地，与当前选择的申请项目明确区分
...
澳大利亚签证通过 ImmiAccount 上传 `JPG` facial image，官方公开说明的文件范围为 70 KB–3.5 MB，并由申请系统提供居中框选；由于没有统一公开的强制像素尺寸，Visa Go 暂不提供固定模板。页面底部的澳大利亚卡片是全局“不支持”说明，不是当前选择结果。
